...
name: ship
description: >-
  Closing pass of the eng pipeline. Takes implemented work and lands it: reviews
  the final diff, splits it into clean atomic commits in the repo's commit style,
  pushes the branch, and opens a PR whose description is generated from the .plan
  doc's problem, scope, acceptance criteria, and work log — then records the PR
  back in the doc and advances phase to `shipped`. Works without a .plan doc too
  (commits + PR from the diff alone). Use when the work is done and needs to land.
  Triggers: "ship it", "ship this", "commit and push", "open the PR", "land this",
  "create a PR".
---
