...
name: grill
description: >-
  Second pass of the eng pipeline. Reads the Open Questions a scope doc already
  recorded, grills the user about them one batch at a time, then adds its own
  engineering and business questions to close remaining ambiguity — exploring the
  codebase a little more if it needs context. Writes answers into the doc's
  Decisions section and sharpens scope/acceptance criteria. Use when a .plan doc
  exists and needs its requirements nailed down. Triggers: "grill me", "ask me
  questions about this", "tighten the requirements", "resolve open questions".
---
